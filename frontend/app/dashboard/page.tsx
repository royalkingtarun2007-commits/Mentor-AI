  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (const layer of layers) {
        ...
        for (let i = 0; i <= 80; i++) {
          const x = (i / 80) * W;
          const y = layer.yBase * H + Math.sin(i * layer.freq * 0.1 + t * layer.speed) * layer.amp * H + Math.sin(i * layer.freq * 0.07 + t * layer.speed * 1.2 + 1) * layer.amp * 0.4 * H;
          ctx!.lineTo(x, y);
        }
        ctx!.lineTo(W, H); ctx!.closePath();
        const g = ctx!.createLinearGradient(0, 0, 0, H);
        g.addColorStop(0, layer.color1); g.addColorStop(1, layer.color2);
        ctx!.fillStyle = g; ctx!.fill();
      }
      for (let i = 0; i < 80; i++) {
        const sx = ((i * 9311) % W), sy = ((i * 6271) % (H * 0.5));
        const tw = (Math.sin(t * 2 + i * 0.8) + 1) / 2;
        ctx!.fillStyle = `rgba(255,255,255,${0.1 + tw * 0.3})`;
        ctx!.beginPath(); ctx!.arc(sx, sy, 0.5 + (i % 2) * 0.3, 0, Math.PI * 2); ctx!.fill();
      }
      const fade = ctx!.createLinearGradient(0, H * 0.4, 0, H);
      fade.addColorStop(0, "transparent"); fade.addColorStop(1, "rgba(3,3,12,1)");
...
export default function Dashboard() {
  ...
  const features = [
    { icon: BookOpen, title: "Knowledge Base", description: "Everything you save becomes searchable intelligence. Notes are vectorized for instant semantic retrieval.", href: "/notes", accent: "#6366f1", accentBorder: "rgba(99,102,241,0.28)", cta: "Open Notes", number: "01" },
    { icon: MessageSquare, title: "AI Chat", description: "Chat with an AI that only knows what you've taught it. Powered by your notes, not the internet.", href: "/chat", accent: "#8b5cf6", accentBorder: "rgba(139,92,246,0.28)", cta: "Start Chat", number: "02" },
    { icon: Sparkles, title: "Smart Summary", description: "Drop in any note and get a structured, AI-generated summary with key points extracted instantly.", href: "/summary", accent: "#06b6d4", accentBorder: "rgba(6,182,212,0.28)", cta: "Try Summary", number: "03" },
  ];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#03030c", color: "#f0f0ff", overflowX: "hidden", position: "relative" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "100px 24px 60px", textAlign: "center" }}>
        {[...Array(10)].map((_, i) => (
          <motion.div key={i} animate={{ y: [0, -28, 0], opacity: [0, 0.5, 0] }} transition={{ duration: 5 + i * 0.6, repeat: Infinity, delay: i * 0.5 }}
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
          ...
          <span style={{ ...mono, color: "#818cf8", fontSize: "11px", letterSpacing: "0.15em", textTransform: "uppercase" }}>AI Online · Groq Connected</span>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.9, ease: [0.16, 1, 0.3, 1] }} style={{ marginBottom: "16px" }}>
          <h1 style={{ ...script, fontSize: "clamp(52px, 9vw, 100px)", fontWeight: 300, fontStyle: "italic", lineHeight: 1 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }}
          ...
          <Link href="/chat" style={{ textDecoration: "none" }}>
            <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
              style={{ display: "flex", alignItems: "center", gap: "10px", padding: "16px 32px", borderRadius: "14px", background: "rgba(4,4,18,0.7)", border: "1px solid rgba(255,255,255,0.12)", color: "#d0d0ff", ...jakarta, fontSize: "15px", fontWeight: 600, cursor: "pointer", backdropFilter: "blur(12px)" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "0 24px 80px" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1px", background: "rgba(255,255,255,0.06)", borderRadius: "20px", overflow: "hidden", border: "1px solid rgba(255,255,255,0.08)" }}>
            {stats.map((stat, i) => (
              <motion.div key={i} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                whileHover={{ background: "rgba(6,6,22,1)" }}
                style={{ display: "flex", alignItems: "center", gap: "14px", padding: "26px", background: "rgba(4,4,18,0.96)", transition: "background 0.2s" }}>
                <div style={{ width: "42px", height: "42px", borderRadius: "12px", background: `${stat.color}14`, border: `1px solid ${stat.color}28`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <stat.icon size={18} style={{ color: stat.color }} />
                </div>
                <div>
                  <p style={{ ...mono, fontSize: "9px", color: "#3a3a5a", textTransform: "uppercase", letterSpacing: "0.15em", marginBottom: "4px" }}>{stat.label}</p>
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                ...
                <Link href={f.href} style={{ textDecoration: "none", display: "block", height: "100%" }}>
                  <motion.div whileHover={{ y: -5 }} transition={{ duration: 0.2 }}
                    style={{ position: "relative", padding: "32px", borderRadius: "22px", background: "rgba(4,4,18,0.9)", border: `1px solid ${f.accentBorder}`, overflow: "hidden", cursor: "pointer", height: "100%", backdropFilter: "blur(20px)" }}>
                    ...
                    <div style={{ position: "absolute", top: "16px", right: "20px", ...script, fontSize: "52px", fontWeight: 300, fontStyle: "italic", color: "rgba(255,255,255,0.05)", lineHeight: 1, userSelect: "none" }}>
                      {f.number}
                    </div>
                    <div style={{ width: "52px", height: "52px", borderRadius: "14px", background: `${f.accent}18`, border: `1px solid ${f.accent}30`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "22px" }}>
                    ...
                    <p style={{ ...jakarta, fontSize: "14px", lineHeight: 1.75, color: "#8080a8", marginBottom: "24px" }}>{f.description}</p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            {steps.map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
                transition={{ delay: i * 0.12 }} whileHover={{ x: 5 }}
                style={{ display: "flex", gap: "24px", padding: "28px 32px", borderRadius: "18px", background: "rgba(4,4,18,0.9)", border: "1px solid rgba(255,255,255,0.09)", alignItems: "center", backdropFilter: "blur(20px)" }}>
                <div style={{ ...script, fontSize: "44px", fontWeight: 300, fontStyle: "italic", color: item.color, opacity: 0.6, flexShrink: 0, lineHeight: 1, width: "68px" }}>{item.step}</div>
                ...
                <div>
                  ...
                  <p style={{ ...jakarta, fontSize: "14px", color: "#6868a0", lineHeight: 1.6 }}>{item.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "0 24px 100px" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            style={{ position: "relative", padding: "64px 48px", borderRadius: "28px", background: "rgba(4,4,18,0.92)", border: "1px solid rgba(99,102,241,0.22)", textAlign: "center", overflow: "hidden", backdropFilter: "blur(24px)" }}>
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "1px", background: "linear-gradient(90deg, transparent, rgba(139,92,246,0.6), rgba(6,182,212,0.5), transparent)" }} />
            <div style={{ position: "absolute", top: 0, left: "50%", transform: "translateX(-50%)", width: "600px", height: "200px", background: "radial-gradient(ellipse at top, rgba(99,102,241,0.07), transparent 70%)", pointerEvents: "none" }} />
            ...
            <p style={{ ...jakarta, fontSize: "16px", color: "#6868a0", marginBottom: "40px", maxWidth: "440px", margin: "0 auto 40px", lineHeight: 1.7 }}>
