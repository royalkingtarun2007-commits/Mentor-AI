...
export default function SummaryPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030308", paddingTop: "96px", paddingBottom: "80px", position: "relative", overflow: "hidden" }}>
      ...
      <motion.div animate={{ scale: [1, 1.1, 1], opacity: [0.4, 0.7, 0.4] }} transition={{ duration: 8, repeat: Infinity }}
      ...
      <motion.div animate={{ scale: [1, 1.08, 1], opacity: [0.3, 0.6, 0.3] }} transition={{ duration: 10, repeat: Infinity, delay: 3 }}
      ...
      <div style={{ position: "relative", maxWidth: "900px", margin: "0 auto", padding: "0 24px" }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: "48px" }}>
          <p style={{ ...mono, fontSize: "10px", color: "#10b981", letterSpacing: "0.3em", textTransform: "uppercase", marginBottom: "12px" }}>AI Feature</p>
          ...
          {notes.map((note, i) => (
            <motion.div key={note.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
              ...
              <div onClick={() => setExpandedId(expandedId === note.id ? null : note.id)}
                ...
                <div style={{ width: "42px", height: "42px", borderRadius: "12px", background: note.ai_summary ? "rgba(16,185,129,0.1)" : "rgba(255,255,255,0.04)", border: `1px solid ${note.ai_summary ? "rgba(16,185,129,0.2)" : "rgba(255,255,255,0.08)"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  {note.ai_summary ? <Sparkles size={18} style={{ color: "#10b981" }} /> : <BookOpen size={18} style={{ color: "#44445a" }} />}
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "10px", flexShrink: 0 }}>
                  <motion.button
                    ...
                    style={{ display: "flex", alignItems: "center", gap: "6px", padding: "8px 16px", borderRadius: "10px", cursor: loadingId === note.id ? "not-allowed" : "pointer", ...mono, fontSize: "11px", fontWeight: 600, background: note.ai_summary ? "rgba(16,185,129,0.1)" : "linear-gradient(135deg, rgba(0,212,255,0.15), rgba(139,92,246,0.15))", color: note.ai_summary ? "#10b981" : "#00d4ff", border: `1px solid ${note.ai_summary ? "rgba(16,185,129,0.2)" : "rgba(0,212,255,0.2)"}` }}
                  >
                    {loadingId === note.id ? (
                  ...
                  <ChevronRight size={16} style={{ color: "#44445a", transform: expandedId === note.id ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.2s" }} />
                </div>
              </div>
                      ...
                      {note.ai_summary && loadingId !== note.id && (
                        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "14px" }}>
                            ...
                            <span style={{ ...mono, fontSize: "10px", color: "#10b981", textTransform: "uppercase", letterSpacing: "0.15em", fontWeight: 700 }}>AI Summary</span>
                          </div>
