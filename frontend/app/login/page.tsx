...
import { Eye, EyeOff, Zap, ArrowRight, Lock, Mail, Wifi } from "lucide-react";
...
export default function LoginPage() {
  ...
  const [serverStatus, setServerStatus] = useState<"waking" | "ready" | "unknown">("unknown");

  const router = useRouter();

  // Wake up Render backend as soon as login page opens
  useEffect(() => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030308", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", padding: "24px" }}>
      ...
      <div style={{ position: "fixed", top: "20px", right: "20px", zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "6px", padding: "6px 12px", borderRadius: "999px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", backdropFilter: "blur(12px)" }}>
          {serverStatus === "waking" && (
            <>
              <motion.span animate={{ opacity: [1, 0.2, 1] }} transition={{ duration: 1, repeat: Infinity }}
                style={{ width: "5px", height: "5px", borderRadius: "50%", background: "#f59e0b", display: "inline-block" }} />
              <span style={{ ...mono, fontSize: "9px", color: "#f59e0b", letterSpacing: "0.1em" }}>Waking server...</span>
            </>
          )}
          {serverStatus === "ready" && (
            <>
              <span style={{ width: "5px", height: "5px", borderRadius: "50%", background: "#10b981", display: "inline-block" }} />
              <span style={{ ...mono, fontSize: "9px", color: "#10b981", letterSpacing: "0.1em" }}>Server ready</span>
            </>
          )}
          {serverStatus === "unknown" && (
            <>
              <span style={{ width: "5px", height: "5px", borderRadius: "50%", background: "#44445a", display: "inline-block" }} />
              <span style={{ ...mono, fontSize: "9px", color: "#44445a", letterSpacing: "0.1em" }}>Connecting...</span>
            </>
          )}
        </div>
      </div>
      ...
      >
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "24px", padding: "40px", boxShadow: "0 32px 80px rgba(0,0,0,0.6)", backdropFilter: "blur(20px)", position: "relative", overflow: "hidden" }}>
          ...
          <form onSubmit={handleLogin} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            ...
            {/* Email */}
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.35 }} style={{ position: "relative" }}>
            ...
            {/* Password */}
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} style={{ position: "relative" }}>
              ...
              <button type="button" onClick={() => setShowPassword(!showPassword)}
                style={{ position: "absolute", right: "14px", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#44445a", padding: "2px", display: "flex" }}
            ...
            <motion.button
              ...
              type="submit"
              disabled={loading}
              style={{ width: "100%", padding: "14px", borderRadius: "12px", background: "linear-gradient(135deg, #00d4ff, #818cf8)", color: "#000", border: "none", cursor: loading ? "not-allowed" : "pointer", ...syne, fontSize: "14px", fontWeight: 700, letterSpacing: "0.03em", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", opacity: loading ? 0.8 : 1, marginTop: "4px" }}
            >
              {loading ? (
                <>
                  <motion.div
                    ...
                    style={{ width: "16px", height: "16px", border: "2px solid rgba(0,0,0,0.2)", borderTop: "2px solid #000", borderRadius: "50%" }}
                  />
                  Authenticating...
                </>
              ) : (
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}
            style={{ textAlign: "center", ...mono, fontSize: "12px", color: "#44445a", marginTop: "24px" }}>
            no account?{" "}
